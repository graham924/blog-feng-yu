...
public class UserInfoServiceImpl extends ServiceImpl<UserInfoDao, UserInfo> implements UserInfoService {
    ...
    public PageResult<UserOnlineDTO> listOnlineUsers(ConditionVO conditionVO) {
        ...
        List<UserOnlineDTO> userOnlineDTOList = sessionRegistry.getAllPrincipals().stream()
                ...
                .collect(Collectors.toList());
        // 执行分页
        int fromIndex = getLimitCurrent().intValue();   // 从那一条数据开始取
        int size = getSize().intValue();        // 每页的数量
        int toIndex = userOnlineDTOList.size() - fromIndex > size ? fromIndex + size : userOnlineDTOList.size();    // 最后会取到第几条数据
